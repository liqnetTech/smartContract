// Specifically request an abstraction for LiqNetToken
const LiqNetToken = artifacts.require('LiqNetToken');

const assertJump = function (error) { // eslint-disable-line
  assert(error.message.search('revert') || error.message.search('invalid opcode'));
};

async function deployToken() {
  return LiqNetToken.new();
}

contract('LiqNetToken', (accounts) => {
  let instance;
  ...
  it('Check that tokens can be transfered', async () => {
    ...
    await instance.transfer(user1, 100, {
      from: owner,
    });
  ...
  it('Check that account can not transfer more tokens then have', async () => {
    try {
      ...
      await instance.transfer(user2, 200, {
        from: user1,
      }); // user1 have 100 tokens
    } catch (error) {
      assertJump(error);
      return;
    }
  ...
  it('Check that account can transfer approved tokens', async () => {
    await instance.mintTokens(user1, 100);
    await instance.approve(user2, 50, {
      from: user1,
    });
    await instance.transferFrom(user1, user3, 50, {
      from: user2,
    });
  ...
  it('Check that tokens can be approved', async () => {
    try {
      await instance.approve(user2, 1000000, {
        from: user1,
      }); // user1 have 0 tokens
    } catch (error) {
      assertJump(error);
  ...
  it('Burn tokens allowed to spend by other user', async () => {
    ...
    await instance.burnFrom(user1, 50, {
      from: user2,
    }); // 200 - 50 = 150
